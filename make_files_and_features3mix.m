...
feature_fname='~/data/mixture-res/FEATURES-timit-jstat-mixture.mat';
...
for mm=1:length(Ms),
    ...
    for (KK=1:ITER)
        ...
        %else
        [FilterBank,SubbandFrequencies]=make_erb_cos_filters(FS0*mlength,FS0,NFB-2,LF,HF);
        for I=1:M,
            myCgrm{I}= generate_subbands(mixme{I},FilterBank);
        end
        myCgrm2= generate_subbands(soundM,FilterBank);
        
        temp_1=myCgrm{1}.^2;
        temp_2=myCgrm{2}.^2;
        %temp_1=blkproc(temp_1,[100 1],@(x)(ones(100,1)*mean(mean(x))));
        %temp_2=blkproc(temp_2,[100 1],@(x)(on es(100,1)*mean(mean(x))));
       ...
       soundM2=collapse_subbands(myCgrm2.*(maskM),FilterBank);
       soundM2=0.03*(soundM2/sqrt(mean((soundM2.^2))));
         
        mfname=sprintf('mix-v1.M.%d.%d.wav',M,KK);
        
        if is_sound
            p = audioplayer(soundM2/max(soundM2), FS);p.play
             pause;
        end
        
        cd (output_audiodata);
        %wavwrite(soundM,FS0,mfname);
        audiowrite(mfname,soundM2,FS0)
