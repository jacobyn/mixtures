...
for I=1:Nab
    for J=1:NTRAIN
        ...
        if isfield(INFOab{I}{J},'jstats')
            ...
            %vecformat={vecformat{1},vecformat{2},vecformat{3},vecformat{4},vecformat{6}};
            %xlabels  ={xlabels{1},xlabels{2},xlabels{3},xlabels{4},xlabels{6}};
            %ylabels  ={ylabels{1},ylabels{2},ylabels{3},ylabels{4},ylabels{6}};
            %titles  ={titles{1},titles{2},titles{3},titles{4},titles{6}};
            %             xlabels={xlabels{1}};
            %             ylabels={ylabels{1}};
            %             titltes={titles{1}};
        end
...
for I=1:min(15,Ap),
    ...
    clim=[-1,1];
    %clim=[];
    clim=[-1*sqrt(mean(wf.*wf)),1*sqrt(mean(wf.*wf))];
...
dp1= (mean(valsN(labels==1,:)));
dp2= (mean(valsN(labels==2,:)));

%     clim=[-max(abs(wf)),max(abs(wf))];
%     clim=[-3,3];
clim=[];
%clim=[-3*sqrt(mean(dp1.*dp1)),3*sqrt(mean(dp1.*dp1))];
figure(173);dataDP1=nori_cell_array_unvectorize(dp1,vecformat);is_colormap_negative=false;
nori_figure_stat_summary_as_cell_array(dataDP1,xleg,yleg,xlabels,ylabels,titles,is_colormap_negative,clim);

figure(174);dataDP2=nori_cell_array_unvectorize(dp2,vecformat);is_colormap_negative=false;
nori_figure_stat_summary_as_cell_array(dataDP2,xleg,yleg,xlabels,ylabels,titles,is_colormap_negative,clim);


figure(175);dataDP3=nori_cell_array_unvectorize(dp1-dp2,vecformat);is_colormap_negative=false;
